      ...
      return {
          ...
          '<div class="pickadate">' +
            ...
            '<div class="pickadate-body">' +
              '<div class="pickadate-main">' +
                ...
                '</ul>' +
                '<ul class="pickadate-cell">' +
                  '<li ng-repeat="d in dates" ng-click="setDate(d)" class="{{d.className}}" ng-class="{\'pickadate-active\': date == d.date}" ng-date="{{d.date}}">' +
                    '{{d.date | date:"d"}}' +
                  '</li>' +
                '</ul>' +
              '</div>' +
        ...
        link: function(scope, element, attrs, ngModel)  {
          ...
          scope.setDate = function(dateObj) {
            ...
            updateCalenderScope(new Date(dateObj.date),false);
          };
          ...
          scope.changeMonth = function (offset) {
            ...
            scope.render(currentDate);            
            updateStudentCalendar(currentDate);
            setTimeout(function(){
            if(angular.element(document.getElementsByClassName("pickadate-active")).length >0)
                updateCalenderScope(new Date(),false);
            else
                updateCalenderScope(undefined,true);
                },300)  ;
          };
